...
* Приложение LundenIlona - это женское приложение 6-в-1, включающее в себя расчет индивидуальной дневной калорийности,
  дневник питания, шагомер, планинг привлекательности, личный гормональный календарь, а также отдельный блок для
  беременных девушек.
...
- Для локального запуска в дефолтном режиме (без указания ключей) браузер запускается в <code>Selenoid</code>,
  приложение в <code>Browserstack</code>
...
Чтобы тесты корректно запустились локально, необходимо предварительно создать:
- файл testing.properties в директории src/test/resources/web.
  В файле необходимо указать:
  - auth.userLogin=*указать тестовую почту пользователя*
  - auth.userName=*указать тестовое имя пользователя*
  - auth.userPass=*указать тестовый пароль пользователя*
- файл testing.properties в директории src/test/resources/mobile.
  В файле необходимо указать:
  - auth.userName=*указать тестовое имя пользователя*
  - auth.authKey=*указать тестовый ключ пользователя*
  - auth.url=*указать ссылку на BrowserStack's hub*
...
- Команда для запуска с указанием ключей deviceHost (browserstack, emulation, real) и driver (remote, local) (у запуска
  мобильных тестов есть особенности, см. раздел "Особенности запуска мобильных тестов" ниже)
  ```bash  
  ./gradlew clean test -DdeviceHost=*указать значение* -Ddriver=*указать значение*
  ```

- Команда для запуска без указания ключей (все тесты запустятся по дефолту на удаленных фермах: WEB на selenoid ферме,
  MOBILE на browserstack ферме)
  ```bash  
  ./gradlew clean test
  ```
...
- Запустить образ Pixel 4 API 30 (Android 11.0 "R"), нажав на кнопку "Start" справа от названия образа (поскольку этот
  образ прописан в owner .properties проекта)
...
- Открыть командную строку и через нее зайти в директорию platform-tools (там, где установлена SDK (например, C:
...
Для того, чтобы тесты можно было запускать независимо от кого-либо, в <code>Jenkins</code> реализована джоба для
удаленного запуска.
...
После завершения прогона автотестов, в соответствии с настройками в <code>Jenkins</code>, сформируется Allure отчет. Для
просмотра отчета необходимо в Jenkins в окне "Build History" нажать на значок Allure, расположенный справа от номера
билда.
...
Для того, чтобы поддерживать концепцию test cases as documentation, в соответствии с настройками в <code>Jenkins</code>,
в проекте реализована интеграция <code>Jenkins</code> с TMS <code>Allure TestOps</code>. Таким образом ручной
тестировщик может создавать в ней ручные тест-кейсы, а автотестировщик может создавать автотесты с Allure шагами и
выгружать их в нее. Для этого необходимо:
